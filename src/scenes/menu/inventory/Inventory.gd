...
func _ready():
	...
	equip_slots[0].slotType = Slot.SlotType.SHIRT
	equip_slots[1].slotType = Slot.SlotType.PANTS
...
func able_to_put_into_slot(slot: Slot):
	...
	if slot.slotType == Slot.SlotType.SHIRT:
		return holding_item_category == "Shirt"
	elif slot.slotType == Slot.SlotType.PANTS:
